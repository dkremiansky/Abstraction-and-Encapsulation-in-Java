...
public class MarkovModel {
    ...
    public String getRandomText(int numChars) {
        ...
        int firstRandomIndex = myRandom.nextInt(myText.length() - numN);
        String firstRandomLetter = myText.substring(firstRandomIndex, firstRandomIndex+numN);
        sb.append(firstRandomLetter);
        ArrayList<String> letters = getFollows(sb.toString());
        for (int k = 0; k < numChars - numN; k++) {
            if(letters.size() == 0)
                break;
            int index = myRandom.nextInt(letters.size());
            String ourTab = letters.get(index);
            sb.append(ourTab);
            letters = getFollows(sb.substring(sb.length()-numN,sb.length()));
        }
        return sb.toString();
    }
